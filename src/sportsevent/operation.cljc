(ns sportsevent.operation
  "OperationActor -- one sports-event operation = one supervised
  actor run, expressed as a langgraph-clj StateGraph. The advisor
  (SportsEventOps-LLM) is sealed into a single node (:advise); its
  proposal is ALWAYS routed through the Event Integrity Governor
  ...
  One graph run = one sports-event operation (intake -> advise ->
  ...
  decision to a human operator (licensed official). The approver
  resumes with `{:approval {:status :approved}}` (or :rejected).
  `:actuation/finalize-ruling` ALWAYS reaches this node when the
  governor is clean -- see `sportsevent.phase`."
  (:require [langgraph.graph :as g]
            [langgraph.checkpoint :as cp]
            [sportsevent.sportseventadvisor :as sportseventadvisor]
            [sportsevent.governor :as governor]
...
(defn build
  ...
  `sportsevent.store/Store`).
  opts:
    :advisor      -- a `sportsevent.sportseventadvisor/Advisor` (default: mock-advisor)
    :checkpointer -- langgraph checkpointer (default: in-mem)"
  [store & [{:keys [advisor checkpointer]
             :or   {advisor      (sportseventadvisor/mock-advisor)
  ...
  (-> (g/state-graph
      ...
      ;; SportsEventOps-LLM inference (the contained intelligence node) -- proposal only.
      (g/add-node :advise
        (fn [{:keys [request]}]
          (let [p (sportseventadvisor/-advise advisor store request)]
            {:proposal p :audit [(sportseventadvisor/trace request p)]})))

      ;; Event Integrity Governor -- independent censor (separate system than the LLM).
      (g/add-node :govern
